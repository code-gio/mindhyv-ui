<script lang="ts">
	...
	const DEFAULTS = getTemplateDefaults('mega-menu-simple') || {};
		...
		<div class="flex h-16 items-center justify-between">
			...
			<div class="hidden items-center gap-x-6 md:flex">
				<div class="relative">
					...
					{#if menuOpen}
						<div
							class="absolute start-0 top-full mt-2 w-screen max-w-4xl rounded-lg border border-gray-200 bg-white p-6 shadow-lg dark:border-neutral-700 dark:bg-primary"
							onmouseenter={() => (menuOpen = true)}
							onmouseleave={() => (menuOpen = false)}
						>
							<div class="grid gap-8 sm:grid-cols-{columns.length}">
								{#each columns as column}
									<div>
										<h3 class="mb-4 text-sm font-semibold text-gray-900 uppercase dark:text-primary-foreground">
											{column.title}
										</h3>
										<div class="space-y-3">
