<script lang="ts">
	...
	interface MegaMenuFullWidthProps {
		brandName?: string;
		menuLabel?: string;
		columns?: MenuColumn[];
		ctaText?: string;
	...
	const DEFAULTS = getTemplateDefaults('mega-menu-full-width') || {};
		...
		<div class="flex h-16 items-center justify-between">
			...
			<div class="hidden items-center gap-x-6 md:flex">
				<div class="relative">
					...
					{#if menuOpen}
						<div
							class="absolute start-1/2 top-full mt-2 w-screen max-w-7xl -translate-x-1/2 rounded-lg border border-gray-200 bg-white p-8 shadow-lg dark:border-neutral-700 dark:bg-primary"
							onmouseenter={() => (menuOpen = true)}
							onmouseleave={() => (menuOpen = false)}
						>
