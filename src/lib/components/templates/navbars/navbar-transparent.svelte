<script lang="ts">
	...
	const DEFAULTS = getTemplateDefaults('navbar-transparent') || {};
...
<header class="absolute inset-x-0 top-0 z-50 w-full">
	<nav class="mx-auto max-w-340 px-4 sm:px-6 lg:px-8">
		...
		{#if mobileMenuOpen}
			<div class="border-t border-border bg-black/50 py-4 backdrop-blur-lg md:hidden">
				<div class="space-y-3">
					{#each links as link}
